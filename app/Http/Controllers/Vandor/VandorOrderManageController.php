...
//        return view('');
    ...
    public function VandorOrderDetailsDelete($id,$shop_id){

        $order = Order::where('id',$id)->where('shop_id', 'LIKE', "%$shop_id%")->first();
        $arr = $order->shop_id;
        ...
        if($co<2){
            $ar = [];
            foreach ($json as $key=>$jsones){
                if($jsones!=$shop_id){
                    array_push($ar,$jsones);
                }
            }
            $order->shop_id = $ar;
            $order->save();
    ...
    public function VandorApprovedOrderManage(){
