...
class VadorStokeGenarateController extends Controller
{
    ...
    public function VandorCategoryWiseStock(Request $request){
        ...
        $id = Auth::guard('vandor')->user()->id;
        $data['info'] = Vandor::where('id',$id)->first();
        $data['product'] = ProductManage::where('cat_id',$request->cate_wise_report)->where('shop_id',$data['info']->shop_id)->where('status','1')->get();
    ...
    public function VandorStockGenaratorProductAjax(Request $request){
        $id = Auth::guard('vandor')->user()->id;
        $data['info'] = Vandor::where('id',$id)->first();
        $product = ProductManage::where('cat_id',$request->cat_id)->where('shop_id',$data['info']->shop_id)->where('status','1')->get();
    ...
    public function VandorProductWiseStockGenare(Request $request){
        ...
        $id = Auth::guard('vandor')->user()->id;
        $data['info'] = Vandor::where('id',$id)->first();
        $data['product'] = ProductManage::where('cat_id',$request->cate_wise_report)->where('id',$request->product_name)->where('shop_id',$data['info']->shop_id)->where('status','1')->get();
