...
//        $store->carat = $request->carat;
    ...
    public function VandorPanelPaymentWidrowDelete($shop_id,$id,$request_id){

        $data['logo'] = Setting::where('id', '1')->first();
        $data['info'] = Admin::where('id', Auth::guard('admin')->user()->id)->first();
        $data['vandor'] = Vandor::where('id', $id)->where('shop_id', $shop_id)->first();
        $shop_id = Vandor::where('id', $id)->where('shop_id', $shop_id)->first()->shop_id;
        $data['panding_request'] = VandorPaymentRequest::where('id',$request_id)->where('shop_id',@$shop_id)->where('user_id',@$data['vandor']->id)->where('status','1')->delete();
    ...
    public function VandorPanelPaymentWithdrawPaySubmit(Request $request){
        ...
        $vandor = Vandor::where('shop_id',$update->shop_id)->first();
        $request_am = VandorPaymentRequest::where('id',$request->EditeId)->first();



        $data =array(
            'emails'=>$vandor->email,
            'req_amount'=>$request_am->request_amoung,
            'app_amount'=>$request_am->approve_amount,
            'shop_image'=>$vandor->shop_image,
        );

        Mail::send('AdminDashboard/VandorWidrow/mail/app_mail', $data, function ($message) use($data){
            $message->from('info@erx.life', 'We Harbazar');
            $message->to($data['emails'], 'Dear Vandor');
            $message->subject('Approve Your Payment Request');
    ...
    public function VandorOrderListView($shop_id){

        $data['logo'] = Setting::where('id','1')->first();
        $data['info'] = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $data['vandor'] = Vandor::where('super_admin_status','1')->where('shop_id',$shop_id)->first();
        $shop_i = Vandor::where('shop_id',$shop_id)->first();
        $data['shop_i'] = Vandor::where('shop_id',$shop_id)->first();
        $data['order']=Order::OrderBy('id','desc')->where('shop_id', 'LIKE', "%$shop_i->shop_id%")->get();



        return view('AdminDashboard.VandorOrder.order_list_view',$data);
    }

    public function VandorApprovedOrderListView($shop_id){
        ...
        $shop_i = Vandor::where('shop_id',$shop_id)->first();
        $data['shop_i'] = Vandor::where('shop_id',$shop_id)->first();
        $oder_details =  OrderDetail::select('order_details.*')->GroupBy('order_details.order_id')->where('shop_id',$shop_id)->where('order_details.shipping_status','2')->where('order_details.order_status','2')->where('order_details.order_complete','2')->pluck('order_id');
        $collection = collect(json_decode($oder_details));
        $arr = $collection->unique();

        $data['order']=Order::wherehas('order_details')->OrderBy('id','desc')->whereIn('id',$arr)->get();

        return view('AdminDashboard.VandorOrder.approve_order_list',$data);
    }

    public function VandorPandingOrderListView($shop_id){
        ...
        return view('AdminDashboard.VandorOrder.panding_order_list',$data);


    }

    public function VandorPanelOrderDetails($id,$shop_id){
        $order_id = $id;
        $data['logo'] = Setting::where('id','1')->first();
        $data['info'] = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $shop_i = Vandor::where('shop_id',$shop_id)->first();
        $shops_id = Vandor::where('shop_id',$shop_id)->first()->shop_id;
        ...
        return view('AdminDashboard.VandorOrder.order_details',$data);
    }

    public function VandorPanelApporveOrderDetails($id,$shop_id){
        $order_id = $id;
        $data['logo'] = Setting::where('id','1')->first();
        $data['info'] = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $shop_i = Vandor::where('shop_id',$shop_id)->first();
        ...
        $data['order'] = Order::with('customer','payments')->where('id',$order_id)->where('shop_id', 'LIKE', "%$shops_id%")->first();
        $data['order_details'] = OrderDetail::where('order_id',$data['order']->id)->where('shop_id',$shops_id)->where('order_status','2')->where('shipping_status','2')->where('order_complete','2')->get();

        return view('AdminDashboard.VandorOrder.order_details',$data);
    }


    public function VandorPanelPandingOrderDetails($id,$shop_id){
        $order_id = $id;
        $data['logo'] = Setting::where('id','1')->first();
        $data['info'] = Admin::where('id',Auth::guard('admin')->user()->id)->first();
        $shop_i = Vandor::where('shop_id',$shop_id)->first();
        ...
        $data['order'] = Order::with('customer','payments')->where('id',$order_id)->where('shop_id', 'LIKE', "%$shops_id%")->first();
        $data['order_details'] = OrderDetail::where('order_id',$data['order']->id)->where('shop_id',$shops_id)->where('order_status','1')->where('shipping_status','1')->where('order_complete','1')->get();

        return view('AdminDashboard.VandorOrder.order_details',$data);
    }

    public function VandorPanelOrderShopStatus($id,$shop_id){
        ...
        return view('AdminDashboard.VandorOrder.order_status',$data);
