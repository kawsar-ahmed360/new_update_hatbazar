                            ...
                            <tr>
                                ...
                                <td>
                                    @if(@$order->order_status==1)
                                        <a href="{{route('CustomerOrderApprove',[@$order->id,$shop_id])}}"><span class="badge badge-success">Click Approve</span></a>
                                    @else
                                        <a href="{{route('CustomerOrderPanding',[@$order->id,$shop_id])}}"> <span class="badge badge-warning">Already Approve</span></a>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if(@$order->order_status==2)
                                        @if(@$order->shipping_status==1)
                                            <a href="{{route('CustomerOrderShippingApprove',[@$order->id,$shop_id])}}"><span class="badge badge-success">Click Approve</span></a>
                                        @else
                                            <a href="{{route('CustomerOrderShippingPanding',[@$order->id,$shop_id])}}"> <span class="badge badge-warning">Already Approve</span></a>
                                        @endif
                                    @else
                                ...
                                <td>
                                    @if(@$order->order_status==2 && @$order->shipping_status==2)
                                        @if(@$order->order_complete==1)
                                            <a href="{{route('CustomerOrderShippingComplete',[@$order->id,$shop_id])}}"><span class="badge badge-success">Click Approve</span></a>
                                        @else
                                            <a href="{{route('CustomerOrderShippingCompletePanding',[@$order->id,$shop_id])}}"><span class="badge badge-warning">Already Approve</span></a>
                                        @endif
                                    @else
                                        <a style="opacity: 0.5"><span class="badge badge-success">Click Approve</span></a>
                                    @endif
